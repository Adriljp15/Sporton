...
const BankInfoModal = ({isOpen, onClose, bank, onSuccess}: TBankInfoModalProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            onSuccess?.()
            onClose();
            toast.success(isEditMode ? "Bank info updated successfully": "Bank infor created successfully")
        } catch (error) {
        ...
        <Modal isOpen={isOpen} onClose={onClose} title="Add New Bank Account">
            <form onSubmit={handleSubmit} className="flex flex-col gap-6">
                ...
                <Button className="ml-auto mt-3 rounded-lg" type="submit" onClick={handleSubmit} disabled={isSubmitting}>
                    {isSubmitting ? "Processing..." : (isEditMode ? "Update Bank" : "Create Bank Info")}</Button>
            </form>
        </Modal>
    );
};
